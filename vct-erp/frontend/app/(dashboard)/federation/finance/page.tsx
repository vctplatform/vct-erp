        ...
        <div className="h-[280px]">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={d.monthlyIncome} margin={{ top: 5, right: 5, left: -10, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: "var(--color-ink-soft)" }} axisLine={false} tickLine={false} tickFormatter={(v: number) => `${(v / 1_000_000_000).toFixed(1)}`} />
              <Tooltip
                contentStyle={{
          ...
          rows={filtered.map((f) => ({
            ...
            type: (
              <span className={cn(
                ...
                f.type === "income" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-rose-500/10 text-rose-600 dark:text-rose-400"
              )}>
                {f.type === "income" ? <ArrowUpRight className="size-3" /> : <ArrowDownRight className="size-3" />}
                {f.type === "income" ? "Thu" : "Chi"}
              </span>
            ),
            category: <span className="text-sm">{f.category}</span>,
            description: <span className="text-sm text-[var(--color-ink-soft)] dark:text-white/60">{f.description}</span>,
            amount: (
              <span className={cn(
