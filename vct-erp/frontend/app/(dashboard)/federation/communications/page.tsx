...
const TYPE_MAP = {
  announcement: { label: "Thông báo", icon: Megaphone, color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-500/10" },
  news: { label: "Tin tức", icon: Newspaper, color: "text-cyan-600 dark:text-cyan-400", bg: "bg-cyan-500/10" },
  event: { label: "Sự kiện", icon: Calendar, color: "text-indigo-600 dark:text-indigo-400", bg: "bg-indigo-500/10" },
};
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="stagger-children grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard icon={Eye} label="Lượt đọc tháng" value={45_200} color="emerald" />
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {filtered.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="group overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-panel)] transition-all duration-300 hover:-translate-y-1 dark:border-white/6 dark:bg-white/[0.025] dark:hover:border-cyan-500/12"
            >
              {/* Color band top */}
              ...
              <div className="p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <h3 className="mt-0.5 font-semibold text-[var(--color-ink)] dark:text-white">{item.title}</h3>
                    </div>
                  </div>
                  <StatusBadge status={item.status} />
                </div>

                <p className="mt-3 line-clamp-2 text-sm text-[var(--color-ink-soft)] dark:text-white/50">
                  {item.content}
                </p>

                <div className="mt-4 flex flex-wrap items-center gap-3 text-xs text-[var(--color-ink-soft)] dark:text-white/40">
                ...
                {item.type === "event" && item.eventDate && (
                  <div className="mt-3 flex items-center gap-3 rounded-xl border border-indigo-500/15 bg-indigo-500/5 px-3 py-2 text-xs dark:border-indigo-400/10">
                    ...
                    {item.eventLocation && (
                      <>
                        <span className="text-[var(--color-ink-soft)] dark:text-white/30">•</span>
                        <span className="text-indigo-600 dark:text-indigo-400">{item.eventLocation}</span>
                      </>
                    )}
                  </div>
                )}
