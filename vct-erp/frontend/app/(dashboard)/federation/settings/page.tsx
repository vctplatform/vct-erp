          ...
          {[
            ...
            <button
              ...
              className={cn(
                ...
                theme === opt.id
                  ? "border-cyan-500/25 bg-cyan-500/5 shadow-[0_0_20px_rgba(6,182,212,0.08)] dark:border-cyan-400/20 dark:bg-cyan-500/[0.06]"
                  : "border-[var(--color-border)] bg-[var(--color-canvas-soft)] hover:border-cyan-500/15 dark:border-white/5 dark:bg-white/[0.02]"
              )}
            >
              <div className={cn(
                "rounded-lg p-2",
                theme === opt.id ? "bg-cyan-500/15 text-cyan-600 dark:text-cyan-400" : "bg-[var(--color-border)] text-[var(--color-ink-soft)] dark:bg-white/5 dark:text-white/35"
              )}>
              ...
              <div>
                <p className={cn(
                  "text-sm font-medium",
                  theme === opt.id ? "text-[var(--color-ink)] dark:text-white" : "text-[var(--color-ink-soft)] dark:text-white/60"
                )}>
                  {opt.label}
                </p>
                <p className="mt-0.5 text-xs text-[var(--color-ink-soft)] dark:text-white/35">
                  {opt.desc}
                </p>
              </div>
            </button>
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          {PROFILE_FIELDS.map((field) => (
            <div key={field.label}>
              <label className="mb-1.5 block text-xs font-medium text-[var(--color-ink-soft)] dark:text-white/45">
                {field.label}
              </label>
              <input
                type="text"
                defaultValue={field.value}
                className="w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-canvas-soft)] px-4 py-2.5 text-sm text-[var(--color-ink)] outline-none transition-colors focus:border-cyan-500/40 dark:border-white/6 dark:bg-white/[0.03] dark:text-white dark:focus:border-cyan-400/30"
              />
            </div>
          ))}
        </div>
      ...
      >
        <div className="space-y-3">
          {PERMISSION_ROLES.map((role) => (
            <div
              key={role.name}
              className="flex items-center justify-between rounded-xl border border-[var(--color-border)] bg-[var(--color-canvas-soft)] p-4 transition-colors hover:bg-[var(--color-border)] dark:border-white/5 dark:bg-white/[0.02] dark:hover:bg-white/[0.03]"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-[var(--color-ink)] dark:text-white">{role.name}</p>
                  <div className="mt-0.5 flex flex-wrap gap-1">
                    {role.permissions.map((perm) => (
                      <span key={perm} className="text-[0.6rem] text-[var(--color-ink-soft)] dark:text-white/35">
                        {perm}
                      </span>
                    ))}
                  </div>
              ...
              <div className="flex items-center gap-3">
                <span className="rounded-full bg-[var(--color-border)] px-2 py-0.5 text-[0.65rem] font-medium text-[var(--color-ink-soft)] dark:bg-white/5 dark:text-white/40">
                  {role.count} người
                </span>
                <ChevronRight className="size-4 text-[var(--color-ink-soft)] dark:text-white/25" />
              </div>
            </div>
          ))}
        </div>
      ...
      <SectionPanel title="Thông tin Hệ thống" kicker="Platform">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={item.label} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-canvas-soft)] p-3 dark:border-white/5 dark:bg-white/[0.02]">
              <p className="text-[0.62rem] uppercase tracking-[0.25em] text-[var(--color-ink-soft)] dark:text-white/30">
                {item.label}
              </p>
              <p className="mt-1 text-sm font-medium text-[var(--color-ink)] dark:text-white">
                {item.value}
              </p>
            </div>
          ))}
