...
function AnimatedNumber({ value }: { value: number }) {
  ...
  return <span ref={ref}>{display.toLocaleString("vi-VN")}</span>;
}
...
export function StatCard({
  ...
  return (
    <div
      className={cn(
        "group relative overflow-hidden rounded-2xl border border-[var(--color-border)] bg-[var(--color-panel)] p-5 transition-all duration-300 hover:-translate-y-0.5",
        "dark:border-white/6 dark:bg-white/[0.025] dark:backdrop-blur-xl",
        "group-hover:shadow-[0_12px_30px_rgba(11,22,42,0.12)]",
        c.glow,
        c.borderGlow,
        className
      )}
    >
      ...
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-white/40 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100 dark:from-white/[0.025]" />

      {/* Shimmer on hover — dark mode only */}
      ...
      <div className="relative">
        ...
        <div className="mt-4">
          <p className="text-3xl font-bold tracking-tight text-[var(--color-ink)] dark:text-white">
            {typeof value === "number" ? <AnimatedNumber value={value} /> : value}
            {suffix && (
              <span className="ml-1.5 text-base font-medium text-[var(--color-ink-soft)] dark:text-white/45">
                {suffix}
              </span>
            )}
          </p>
          <p className="mt-1.5 text-sm text-[var(--color-ink-soft)] dark:text-white/50">
            {label}
          </p>
        </div>
