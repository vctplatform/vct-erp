...
func New(cfg config.Config, logger *slog.Logger, modules ...httputil.Module) (*Server, error) {
	...
	if storageDriver == "postgres" && cfg.PostgresURL != "" {
		...
		// Athlete Core Module (v3.0 DB Bridge)
		s.Core.Athlete = athlete.NewService(
			athletemod.NewPgRepository(cachedStore.Base().(*store.PostgresStore)),
		)
		logger.Info("athlete core service re-created with v3.0 database repo (core.global_athletes)")
