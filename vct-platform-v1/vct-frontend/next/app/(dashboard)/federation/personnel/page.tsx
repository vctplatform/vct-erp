...
const ROLE_MAP = {
  coach: { label: "Huấn luyện viên", color: "text-cyan-400", bg: "bg-cyan-500/10" },
  referee: { label: "Trọng tài", color: "text-amber-400", bg: "bg-amber-500/10" },
  board_member: { label: "Ban Chấp hành", color: "text-indigo-400", bg: "bg-indigo-500/10" },
  secretary: { label: "Thư ký", color: "text-emerald-400", bg: "bg-emerald-500/10" },
  treasurer: { label: "Thủ quỹ", color: "text-rose-400", bg: "bg-rose-500/10" },
};
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="stagger-fed grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard icon={GraduationCap} label="Huấn luyện viên" value={3456} color="cyan" />
        ...
        <StatCard icon={Award} label="Chứng chỉ Quốc gia" value={2840} color="emerald" />
      </div>
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {filtered.map((person) => {
          const role = ROLE_MAP[person.role as keyof typeof ROLE_MAP];
          ...
          return (
            ...
            >
              <div className="flex items-start gap-4">
                {/* Avatar */}
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-500/20 to-indigo-500/10 text-lg font-bold text-cyan-300">
              ...
              <div className="mt-4 flex items-center gap-4 text-sm">
                <div className="flex items-center gap-2">
                  <span className="h-4 w-4 rounded-md border border-white/10" style={{ backgroundColor: belt.color }} />
                  <span className="text-xs text-white/60">{belt.label}</span>
                </div>
              ...
              <div className="mt-3 flex flex-wrap gap-1.5">
                {person.certifications.map((cert) => (
                  <span key={cert} className="inline-flex items-center gap-1 rounded-lg border border-emerald-500/15 bg-emerald-500/5 px-2 py-0.5 text-[0.65rem] font-medium text-emerald-400">
              ...
              {person.specialization && (
                <p className="mt-3 text-xs text-white/40">
                  Chuyên môn: <span className="text-white/70">{person.specialization}</span>
                </p>
              )}
