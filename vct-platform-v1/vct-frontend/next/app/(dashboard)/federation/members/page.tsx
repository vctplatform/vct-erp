  ...
  return (
    <section className="space-y-6">
      ...
      <div className="stagger-fed grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard icon={Users} label="Đang hoạt động" value={45120} color="emerald" />
              ...
              return {
                ...
                name: (
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-cyan-500/20 to-teal-500/10 text-xs font-bold text-cyan-300">
                      {m.fullName.split(" ").pop()?.charAt(0)}
                    </div>
                    <div>
                      <p className="font-medium text-white">{m.fullName}</p>
                      <p className="text-xs text-white/40">{m.martialArt}</p>
                    </div>
                  </div>
                ),
                belt: (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-white/70">{belt.label}</span>
                  </div>
                ),
                club: <span className="text-sm text-white/80">{m.clubName}</span>,
                province: (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {paginatedMembers.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="group relative overflow-hidden rounded-2xl border border-white/6 bg-white/[0.025] p-5 transition-all duration-300 neon-hover"
              >
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-cyan-500/20 to-teal-500/10 text-sm font-bold text-cyan-300">
                    {m.fullName.split(" ").pop()?.charAt(0)}
                  </div>
                  <div className="min-w-0">
                    <p className="truncate font-medium text-white">{m.fullName}</p>
                    <p className="font-mono text-[0.65rem] text-cyan-400">{m.memberId}</p>
                  </div>
                  <Link href={`/federation/members/${m.id}`} className="ml-auto rounded-lg p-1.5 opacity-0 transition-opacity hover:bg-white/5 group-hover:opacity-100">
                ...
                <div className="mt-4 flex items-center gap-2">
                  ...
                  <span className="text-xs text-white/50">{m.martialArt}</span>
                </div>
