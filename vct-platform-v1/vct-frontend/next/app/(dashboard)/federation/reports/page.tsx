        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {REPORT_TEMPLATES.map((report) => {
            ...
            return (
              <div
                key={report.id}
                className="group relative cursor-pointer overflow-hidden rounded-xl border border-white/5 bg-white/[0.02] p-5 transition-all duration-300 neon-hover"
              >
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div className="flex gap-1.5">
                    {report.formats.map((fmt) => (
                      <span key={fmt} className="rounded border border-white/6 px-1.5 py-0.5 text-[0.6rem] font-medium text-white/40">
                        {fmt}
                      </span>
                    ))}
                  </div>
                  <div className="flex items-center gap-1.5">
                    <button className="rounded-lg p-1.5 text-white/35 transition-colors hover:bg-white/5">
