...
import {
  ...
  Clock, GraduationCap, Award, Activity, Wallet,
} from "lucide-react";
import {
...
export default function FederationDashboardPage() {
  ...
  return (
    <section className="space-y-6">
      ...
      {/* KPI Cards */}
      <div className="stagger-fed grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          icon={Users}
          label="Tổng hội viên"
      ...
      {/* Overview Ribbon */}
      <div className="animate-fed-slide-up grid grid-cols-2 gap-3 rounded-2xl border border-white/6 bg-white/[0.02] p-4 sm:grid-cols-4">
        {[
          { label: "Đang hoạt động", value: "4", icon: Activity, color: "text-emerald-400" },
          { label: "Chờ duyệt", value: "23", icon: Clock, color: "text-amber-400" },
          { label: "Thu Q4", value: "3.2 tỷ", icon: Wallet, color: "text-cyan-400" },
          { label: "Huy chương đã trao", value: "1,247", icon: Award, color: "text-indigo-400" },
        ].map((item) => (
          <div key={item.label} className="flex items-center gap-3 px-2">
            <item.icon className={cn("size-5", item.color)} />
            <div>
              <p className="text-lg font-bold text-white">{item.value}</p>
              <p className="text-[0.65rem] text-white/40">{item.label}</p>
            </div>
          </div>
        ))}
      </div>
            ...
            {d.alerts.map((alert) => {
              ...
              return (
                ...
                >
                  ...
                  {alert.actionLabel && (
                    <button className="shrink-0 rounded-lg bg-white/5 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-white/10">
                      {alert.actionLabel}
                    </button>
                  )}
                </div>
              ...
              return (
                <div key={activity.id} className="flex gap-3">
                  ...
                  <div className="pb-4">
                    ...
                    <p className="mt-1 flex items-center gap-1 text-[0.65rem] text-white/35">
            ...
            rows={d.topClubs.map((club) => ({
              rank: (
                <span className={cn(
                  ...
                  club.rank === 1 && "bg-amber-500/15 text-amber-400",
                  club.rank === 2 && "bg-slate-300/20 text-slate-300",
                  club.rank === 3 && "bg-orange-500/15 text-orange-400",
                  club.rank > 3 && "text-white/50"
                )}>
                  {club.rank}
                </span>
              ),
              club: (
                <div className="flex items-center gap-2">
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500/15 to-teal-500/10 text-xs font-bold text-cyan-400">
              ...
              trend: (
                <span className={cn(
                  ...
                  club.trend === "up" && "text-emerald-400",
                  club.trend === "down" && "text-rose-400",
            ...
            {topBelts.map((item) => {
              ...
              return (
                <div key={item.belt} className="flex items-center gap-3">
                  ...
                  />
                  <span className="w-20 shrink-0 text-sm text-white/80">
                    {belt.label}
                  </span>
                  ...
                  <span className="w-16 text-right text-xs font-medium text-white/50">
