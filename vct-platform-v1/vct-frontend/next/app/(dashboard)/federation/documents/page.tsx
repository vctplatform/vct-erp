...
const PRIORITY_MAP = {
  ...
  urgent: { label: "Khẩn", color: "text-amber-400", bg: "bg-amber-500/10" },
  critical: { label: "Hỏa tốc", color: "text-rose-400", bg: "bg-rose-500/10" },
};
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="space-y-4">
        {filtered.map((doc) => {
          const priority = PRIORITY_MAP[doc.priority as keyof typeof PRIORITY_MAP];
          ...
          return (
            <div
              key={doc.id}
              className="group rounded-2xl border border-white/6 bg-white/[0.025] p-5 transition-all duration-300 neon-hover"
            >
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="flex items-start gap-4">
                  <div className="rounded-xl bg-gradient-to-br from-cyan-500/10 to-teal-500/5 p-3 text-center">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1.5 flex flex-wrap items-center gap-3 text-xs text-white/45">
                      ...
                      <span>•</span>
                      <span>{TYPE_MAP[doc.type as keyof typeof TYPE_MAP]}</span>
                      <span>•</span>
                    ...
                    {doc.attachments.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-2 text-center">
                        {doc.attachments.map((file) => (
                          <span key={file} className="inline-flex items-center gap-1 rounded-lg border border-white/5 bg-white/[0.02] px-2 py-1 text-[0.65rem]">
                            <Paperclip className="size-3 text-white/35" />
              ...
              {doc.approvalChain.length > 0 && (
                <div className="mt-4 rounded-xl border border-white/5 bg-white/[0.02] p-4">
                  <div className="mb-3 flex items-center justify-between">
                    <p className="text-[0.62rem] uppercase tracking-[0.25em] text-white/35">
                      Quy trình phê duyệt
                    </p>
                  ...
                  <div className="flex items-center gap-2">
                    {doc.approvalChain.map((step, i) => (
                      <div key={step.order} className="flex items-center gap-2 text-center">
                        <div className={cn(
                          ...
                          step.status === "approved"
                            ...
                            : "border border-white/5 bg-white/[0.03] text-white/40"
                        )}>
