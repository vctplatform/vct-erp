  ...
  return (
    <section className="space-y-6">
      ...
      <div className="stagger-fed grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard icon={Users} label="Thí sinh năm nay" value={4526} color="emerald" />
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 rounded-xl border border-white/5 bg-white/[0.02] p-3">
                    <div className="flex items-center gap-2">
                      <span className="h-6 w-6 rounded-lg border border-white/12" style={{ backgroundColor: currentBelt.color }} />
                      <span className="text-sm font-medium text-white">{currentBelt.label}</span>
                    </div>
                    <ArrowRight className="size-4 text-cyan-400" />
                    <div className="flex items-center gap-2">
                      <span className="h-6 w-6 rounded-lg border border-white/12" style={{ backgroundColor: targetBelt.color }} />
                      <span className="text-sm font-medium text-white">{targetBelt.label}</span>
                    </div>
                  </div>
                  ...
                  {exam.judges.length > 0 && (
                    <div>
                      <p className="mb-2 text-[0.62rem] uppercase tracking-[0.25em] text-white/35">
                        Hội đồng Giám khảo
                      </p>
                      <div className="flex flex-wrap gap-2">
                        {exam.judges.map((j) => (
                          <span key={j.id} className="inline-flex items-center gap-2 rounded-lg border border-white/6 bg-white/[0.03] px-3 py-1.5 text-xs">
                            <span className="font-medium text-white">{j.name}</span>
                            <span className="text-white/40">({j.role === "chief" ? "Chủ khảo" : "Uỷ viên"})</span>
                          </span>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="text-center">
                        <p className="text-4xl font-bold text-white">
                          {passRate}<span className="text-lg text-white/50">%</span>
                        </p>
                        <p className="mt-1 text-xs text-white/40">Tỷ lệ đạt</p>
                      </div>
                      <ProgressBar value={exam.passedCount} max={exam.candidateCount} color="emerald" />
                      <div className="grid grid-cols-3 gap-3 text-center">
                        <div>
                          <p className="text-lg font-bold text-white">{exam.candidateCount}</p>
                          <p className="text-[0.6rem] text-white/35">Dự thi</p>
                        </div>
                        <div>
                          <p className="text-lg font-bold text-emerald-400">{exam.passedCount}</p>
                          <p className="text-[0.6rem] text-white/35">Đạt</p>
                        </div>
                        <div>
                          <p className="text-lg font-bold text-rose-400">{exam.failedCount}</p>
                          <p className="text-[0.6rem] text-white/35">Rớt</p>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center justify-center py-6 text-center">
                      <Clock className="size-8 text-white/25" />
                      <p className="mt-3 text-sm font-medium text-white">Chưa có kết quả</p>
