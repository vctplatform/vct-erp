  ...
  return (
    ...
    >
      ...
      <div className={cn("mb-5 flex items-start justify-between gap-4", noPadding && "px-5 pt-5 md:px-6 md:pt-6")}>
        <div>
          {kicker && (
            <p className="mb-1.5 text-[0.62rem] uppercase tracking-[0.3em] text-white/40">
              {kicker}
            </p>
          )}
          ...
          <tr className="border-b border-white/10">
            {columns.map((col) => (
              <th
                key={col.key}
                className={cn(
                  "whitespace-nowrap px-4 py-3 text-[0.68rem] font-semibold uppercase tracking-[0.2em] text-white/40",
          ...
          {rows.map((row, i) => (
            <tr
              ...
              className={cn(
                ...
                zebraStripe && i % 2 === 1 && "bg-white/[0.01]",
                "hover:bg-white/[0.03]",
                compact ? "text-sm" : ""
              )}
            >
              {columns.map((col) => (
                <td
                  key={col.key}
                  className={cn(
                    "whitespace-nowrap px-4 text-white/80",
...
export function PageHeader({
  ...
  return (
    <div className="animate-fed-fade-in mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        {kicker && (
          <p className="mb-1.5 text-[0.62rem] uppercase tracking-[0.3em] text-cyan-400/60">
            {kicker}
          </p>
        )}
...
export function SearchInput({
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-2 transition-colors focus-within:border-cyan-400/30",
        className
      )}
    >
...
// ... other components from shared.tsx (Tabs, FilterChip, etc.)
    ...
    <button
      ...
      className={cn(
        ...
        selected
          ...
          : "border border-white/6 bg-white/[0.03] text-white/50 hover:bg-white/6"
      )}
    >
      {label}
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <div className="flex items-center gap-1.5 text-xs font-medium text-white/40">
        ...
        Bộ lọc
        {activeCount > 0 && (
          <span className="rounded-full bg-cyan-500/15 px-1.5 py-0.5 text-[0.6rem] font-bold text-cyan-400">
            {activeCount}
          </span>
        )}
      </div>
    ...
    <div className="flex flex-col items-center gap-3 pt-4 sm:flex-row sm:justify-between">
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          className="rounded-lg p-1.5 text-white/40 transition-colors hover:bg-white/5 disabled:opacity-30"
        >
